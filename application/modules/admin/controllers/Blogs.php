...
class Blogs extends MY_Controller {
	...
	public function index(){
		...
		$config['total_rows'] = count($this->Blog->find());
		$config['per_page'] = 10;
        ...
        if ($this->input->get('q')):
            $q = $this->input->get('q');
            $this->data['posts'] = $this->Blog->find($config['per_page'], $this->uri->segment(4),$user_id, $q);
            ...
            $this->data['posts'] = $this->Blog->find($config['per_page'], $this->uri->segment(4),$user_id);
        endif;
	...
	public function add(){
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $remember = (bool) $this->input->post('show_home_page');
            if($remember){
            ...
            $this->session->set_flashdata('message', message_box('New blog has been saved','success'));
            redirect('admin/blogs');
        }
        $this->data['categories'] = $this->Category->find_list();
        $this->data['tags'] = $this->Tag->find_list();
       	$this->load_admin('blogs/add');
	}
	...
	public function edit($id = null){
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $this->session->set_flashdata('message', message_box('Blog has been saved','success'));
            redirect('admin/blogs');
        }
        $this->data['post'] = $this->Blog->find_by_id($id);
        $this->data['categories'] = $this->Category->find_list();
        $this->data['tags'] = $this->Tag->find_list();
